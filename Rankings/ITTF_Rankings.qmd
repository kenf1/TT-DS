...
This script was written as a continuation of the [ITTF Rankings 2001-2020 project](https://github.com/kenf1/Kaggle_ITTF). Instead of having the data readily available in .csv format, I took on the task of collecting the data myself using webscraping.
...
datasetYear = re.sub(r"[^a-zA-Z0-9]","",datasetYear)

#extract competition
datasetEvent = re.split("\\d{4}",rankingType)[0]
...
#save to csv
ittf_rank.to_csv("ITTF "+fullName+".csv",index=False)
```
